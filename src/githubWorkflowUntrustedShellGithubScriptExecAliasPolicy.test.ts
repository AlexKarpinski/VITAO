...
const collectExecutionAliases = (script: string) => {
  ...
  for (const match of script.matchAll(/\b(?:const|let|var)\s*\{([^}]+)\}\s*=\s*require\(\s*['"](?:node:)?child_process['"]\s*\)/g)) {
    for (const entry of match[1].split(',')) {
      ...
      aliases.set(pair[2] ?? pair[1], pair[1]);
    }
  }

  for (const match of script.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*require\(\s*['"](?:node:)?child_process['"]\s*\)\s*\.\s*(exec|execSync|execFile|execFileSync|spawn|spawnSync)\b/g)) {
    aliases.set(match[1], match[2]);
  }

  const assignments = [...script.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*)\s*;?/g)];
  let changed = true;
  while (changed) {
    changed = false;
...
const hasAliasedUntrustedShellExecution = (script: string) => {
  ...
  for (const [alias, api] of aliases) {
    for (const rawArgs of extractCallArgs(script, alias)) {
      ...
      const explicitShell = executable !== null && shellExecutable.test(executable);
      if (explicitShell) {
...
const expectNoAliasedGithubScriptShellExecution = (workflow: string, source: string) => {
  for (const script of collectGithubScriptBodies(workflow)) {
    expect(
      hasAliasedUntrustedShellExecution(script),
      `${source}: aliased child-process API executes attacker-controlled event text`,
    ).toBe(false);
  }
};

describe('GitHub Script aliased shell API trust boundary', () => {
  it('scans every checked-in workflow', () => {
    expect(workflowFiles.length).toBeGreaterThan(0);
    for (const file of workflowFiles) {
      expectNoAliasedGithubScriptShellExecution(readFileSync(join(workflowsDir, file), 'utf8'), file);
    }
  });

  it('rejects a destructured execSync alias fed comment text', () => {
    const unsafe = [
      'jobs:',
      '  test:',
      '    steps:',
  ...
  it('allows aliased execFileSync when payload is only data for a non-shell executable', () => {
    ...
    expectNoAliasedGithubScriptShellExecution(safe, 'safe.yml');
  });
});
