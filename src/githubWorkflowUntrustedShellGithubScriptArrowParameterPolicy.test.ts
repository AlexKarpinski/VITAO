...
const collectGithubScriptBodies = (workflow: string) => {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const uses = lines[index].replace(/\s+#.*$/, '').match(/^(\s*)-?\s*uses\s*:\s*['"]?actions\/github-script@[^\s'"]+['"]?\s*$/i);
    if (!uses) continue;
...
type ArrowInfo = { name: string; params: string[]; body: string };

const collectArrowFunctions = (script: string): ArrowInfo[] => {
  const arrows: ArrowInfo[] = [];
  const matcher = /\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:\(([^)]*)\)|([A-Za-z_$][\w$]*))\s*=>\s*([^;\n]+(?:;|$))/g;
  for (let match = matcher.exec(script); match; match = matcher.exec(script)) {
    const params = (match[2] ?? match[3] ?? '')
      .split(',')
      .map((param) => param.trim())
      .filter(Boolean);
    arrows.push({ name: match[1], params, body: match[4] });
  }
  return arrows;
};
...
const calledWithUntrustedParam = (script: string, fn: ArrowInfo) => {
...
const arrowExecutesTaintedParam = (fn: ArrowInfo, tainted: Set<string>) => {
...
const hasUntrustedArrowParameterExecution = (script: string) =>
  collectArrowFunctions(script).some((fn) => arrowExecutesTaintedParam(fn, calledWithUntrustedParam(script, fn)));

const expectNoArrowParameterShellExecution = (workflow: string, source: string) => {
  for (const script of collectGithubScriptBodies(workflow)) {
    expect(
      hasUntrustedArrowParameterExecution(script),
      `${source}: GitHub Script passes attacker-controlled event text through an arrow function into shell execution`,
    ).toBe(false);
  }
};

describe('GitHub Script arrow-function shell trust boundary', () => {
  it('scans every checked-in workflow', () => {
    expect(workflowFiles.length).toBeGreaterThan(0);
    for (const file of workflowFiles) {
      expectNoArrowParameterShellExecution(readFileSync(join(workflowsDir, file), 'utf8'), file);
    }
  });

  it('rejects payload text passed through an arrow parameter to execSync', () => {
    const unsafe = [
      'jobs:',
      '  test:',
      '    steps:',
    ...
    expect(() => expectNoArrowParameterShellExecution(unsafe, 'unsafe.yml')).toThrow();
  });

  it('rejects parenthesized arrow parameters passed to an explicit shell', () => {
    const unsafe = [
      'jobs:',
      '  test:',
      '    steps:',
      ...
      '          script: |',
      "            const cp = require('node:child_process');",
      "            const run = (command) => cp.spawnSync('/usr/bin/bash', ['-c', command]);",
      '            run(context.payload.issue.body);',
    ].join('\n');
    expect(() => expectNoArrowParameterShellExecution(unsafe, 'unsafe-shell.yml')).toThrow();
  });

  it('allows an arrow helper that treats attacker text only as data', () => {
    const safe = [
      'jobs:',
      '  test:',
      '    steps:',
      ...
      '          script: |',
      "            const cp = require('node:child_process');",
      "            const print = value => cp.execFileSync('/usr/bin/printf', ['%s', value]);",
      '            print(context.payload.comment.body);',
    ].join('\n');
    expectNoArrowParameterShellExecution(safe, 'safe.yml');
  });
});
